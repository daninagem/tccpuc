...
DEFAULT_INPUT = DATASET_DIR / "pnadc_ocupados_2015_2025_base.parquet"
...
def add_missing_derived_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "categoria_trabalho" in d.columns:
        cat = d["categoria_trabalho"].astype("string").str.strip()
...
def get_available_parquet_columns(path: str | Path) -> set[str]:
    return set(pq.ParquetFile(path).schema_arrow.names)

def get_read_columns(input_path: str | Path, value_col: str, weight_col: str) -> list[str]:
    wanted = [
        "ano", "idade", weight_col, value_col,
        "sexo_desc", "sexo_label", "sexo",
        "raca_cor_analitica", "raca_cor_label", "raca_cor",
        "escolaridade_desc", "escolaridade_desc_analitica", "escolaridade_label", "escolaridade",
        "grupo_h1",
        "formal_ampliado",
        "carteira_assinada",
        "ocup_h3",
        "atividade_h5",
        "posicao_ocupacao_desc", "vd4009_label", "categoria_trabalho",
        "cbo_detalhe_label", "v4010_cbo",
        "uf_sigla",
    ]
    available = get_available_parquet_columns(input_path)
    seen = set()
    return [c for c in wanted if c in available and not (c in seen or seen.add(c))]
...
def bloco_h7_modo(
    ...
    tab = (
        d.groupby(["ano", "grupo_h1"], dropna=False, observed=False)[weight_col]
        .sum()
        .rename("cadeiras_est")
        .reset_index()
    ...
    total = tab.sum(axis=0).sort_values(ascending=False)
    ...
    pop_share = (
        df_table.groupby(["ano", "grupo_h1"], dropna=False, observed=False)[weight_col]
        .sum()
        .rename("ocupados_est")
        .reset_index()
    ...
    pop_share = pop_share.div(pop_share.sum(axis=1), axis=0) * 100
...
def main() -> None:
    ...
    read_cols = get_read_columns(args.input, args.value_col, args.weight_col)
    ...
    # =========================================================
    import json
    map_path = Path(args.input).parent / "category_maps.json"
    ...
    if map_path.exists():
        ...
        for col, mapping in maps.items():
            if col in df_raw.columns:
                reverse_map = {int(k): v for k, v in mapping.items()}
                df_raw[col] = df_raw[col].astype("int32").map(reverse_map)

        print("[INFO] Labels restauradas a partir de category_maps.json")
    else:
        print("[INFO] category_maps.json não encontrado — usando base original")
